...
#     print("The key values pairs are : ", d.items())
#     print("The values is: ", val )
    keys=[]
    for t, v in d.items():
        if (np.array(v) == np.array(val)).all():
#             print("Matched Key is: " , t)
            keys.append(t)
            break
...
#                 temp_center_img = cv2.resize(temp_center_img, (128,64))
#                 fd, hog_image = hog(channel_axis=-1,image = temp_center_img , orientations=9, pixels_per_cell=(8, 8),cells_per_block=(2, 2), visualize=True)
...
#             for cnt in center_mat:
#                 print("Centers are:", cnt)
#             for cnt in coord_mat:
#                 print("Image Segments are: ", cnt)
...
#                     print("Shape is: ", np.array(center_mat[center]).shape)

#                     cos_sim = dot(coord_mat[coord][0], center_mat[center])/(norm(coord_mat[coord][0])*norm(center_mat[center]))
#                     print("COORD O is: ", coord_mat[coord][0])
                    new_matrix = [abs(a_i - b_i)**2 for a_i, b_i in zip(coord_mat[coord][0], center_mat[center])]
#                     print("Distance is : ",np.sum(np.sqrt(new_matrix)))
#                     print("New matrix is : ", new_matrix)
#                     print("Shape of the new matrix is: ", new_matrix.shape)
...
#                 if peoples < len(boxes)-1 and min_val>10000 :# 
#                     print("M I N V A L IS : ", min_val)
# #                     print(len(boxes))
# #                     print(peoples)
#                     peoples+=1
#                     final_center_list['c_person' + str(peoples) ] = tmp_near_pt
#                     final_center_list2['c_person' + str(peoples) ] = tmp_near_pt2    
...
# data1 = pd.read_csv('./ImageReport5/person0.txt')
# data2 = pd.read_csv('./ImageReport5/person1.txt')
# data3 = pd.read_csv('./ImageReport5/person2.txt')
# data1.columns= ['Frame', 'Left', 'Top', 'Right', 'Bottom', 'GazeX', 'GazeY']
# data2.columns= ['Frame', 'Left', 'Top', 'Right', 'Bottom', 'GazeX', 'GazeY']
# data3.columns= ['Frame', 'Left', 'Top', 'Right', 'Bottom', 'GazeX', 'GazeY']
...
#     f1 = data1.loc[data1['Frame'] == img]
#     f2 = data2.loc[data2['Frame'] == img]
#     f3 = data3.loc[data3['Frame'] == img]
#     f4 = data4.loc[data4['Frame'] == img]
    count = 0
